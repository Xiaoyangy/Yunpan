...
//    alert("111");
    ...
    $('input[type=password]').focusin(function(){
        $(this).css('border','1px solid #3F89EC');
        $(this).parent().siblings('.mes').find('.error').hide().end().find('.pwd_tip').show();
    }).focusout(function(){  
        $(this).css('border','1px solid #ddd');  
        $(this).parent().siblings('.mes').find('.pwd_tip').hide();
    });  
    ...
    var isUsr = function(){  
        return /^[0-9A-Za-z\u4e00-\u9fa5]{3,14}$/.test($('#usr').val());
    }  
    ...
    $('#pwd').keyup(function(){  
        if(checkLength()){  
            ...
            $(this).parent().siblings('.mes').find('.pwd_tip').eq(0).css('color','#999');
        }  
    ...
    var checkLength = function(){  
        return $('#pwd').val().length >= 3 && $('#pwd').val().length <=16;
    }  ;
