...
function download(obj, path){
    var url = "file/Privatedownload.action?";
...
function deleteall() {
    ...
    if($id.length < 1){
        ...
        layer.confirm('确认删除？', {
            ...
            $.ajax({
                type:"POST",
                url:"file/delPriDirectory.action",
                data:{
...
function openFile(obj) {
    ...
    if (fileType == "folder-open") {
        ...
        $(obj).attr({"href":"file/openPrifile.action?"+url,"data-lightbox":"test","data-title":fileName});
        return true;
